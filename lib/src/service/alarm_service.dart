...
class AlarmService {
  ...
  Future<Alarm> saveAlarm(Alarm alarm, {RequestConfig? requestConfig}) async {
    var response = await _tbClient.post<Map<String, dynamic>>('/api/alarm', data: jsonEncode(alarm),
        options: defaultHttpOptionsFromConfig(requestConfig));
    return Alarm.fromJson(response.data!);
  }
  ...
  Future<void> deleteAlarm(String alarmId, {RequestConfig? requestConfig}) async {
    await _tbClient.delete('/api/alarm/$alarmId',
        options: defaultHttpOptionsFromConfig(requestConfig));
  }

  Future<PageData<AlarmInfo>> getAlarms(AlarmQuery query, {RequestConfig? requestConfig}) async {
    var queryParams = query.toQueryParameters();
    var response = await _tbClient.get<Map<String, dynamic>>('/api/alarm/${query.affectedEntityId!.entityType.toShortString()}/${query.affectedEntityId!.id}', queryParameters: queryParams,
        options: defaultHttpOptionsFromConfig(requestConfig));
    return _tbClient.compute(parseAlarmInfoPageData, response.data!);
  }

  Future<PageData<AlarmInfo>> getAllAlarms(AlarmQuery query, {RequestConfig? requestConfig}) async {
    var queryParams = query.toQueryParameters();
    var response = await _tbClient.get<Map<String, dynamic>>('/api/alarms', queryParameters: queryParams,
        options: defaultHttpOptionsFromConfig(requestConfig));
    return _tbClient.compute(parseAlarmInfoPageData, response.data!);
  }

  Future<AlarmSeverity?> getHighestAlarmSeverity(EntityId entityId, {AlarmSearchStatus? alarmSearchStatus,
                                                AlarmStatus? alarmStatus, RequestConfig? requestConfig}) async {
    ...
    var response = await _tbClient.get<String>('/api/alarm/highestSeverity/${entityId.entityType.toShortString()}/${entityId.id}',
        queryParameters: queryParams, options: defaultHttpOptionsFromConfig(requestConfig));
    return response.data != null ? alarmSeverityFromString(response.data!) : null;
  }
